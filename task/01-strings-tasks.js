...
function getRectangleString(width, height) {
    let row = new Array(width).fill('│').fill(' ', 1, width - 1).join('') + '\n';
    let firstRow = '┌' + new Array(width - 2).fill('─').join('') + '┐\n';
    let lastRow = '└' + new Array(width - 2).fill('─').join('') + '┘\n';
...
function encodeToRot13(str) {
    let charCodes = str.split('').map(letter => letter.charCodeAt(0));
    charCodes.forEach(function (code, i) {
        if ((64 < code && code < 78) || (96 < code && code < 110)) {
            charCodes[i] = code + 13;
        }
        if ((77 < code && code < 91) || (109 < code && code < 123)) {
            charCodes[i] = code - 13;
        }
    });
